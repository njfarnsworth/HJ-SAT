from set_up import chunk_0, map, tuple_to_litmap
from pysat.solvers import Solver

solver = Solver(name='glucose4')

literals_in_chunk0 = set()
x1_in_lits = False  # use this to determine whether we'll symmetry block x_1

for cl in chunk_0: # collect all of the literals 
    literals = [map[tuple(c)] for c in cl] # convert all to literals via my integer mapping
    literals_in_chunk0.update(literals)

lit_map = {orig: i+1 for i, orig in enumerate(literals_in_chunk0)} # generate the lit map
rev_map = {v: k for k, v in lit_map.items()} # generate the reverse map

if 1 in literals_in_chunk0:
    print("Chunk 0: Performing symmetry blocking on x_1")
    x1_in_lits = True
    
print("Chunk 0 literals before map: ", literals_in_chunk0)
print("Chunk 0 literals after map: ", lit_map)
...
print(f"Found {solution_count} solutions in chunk 0\n\n")